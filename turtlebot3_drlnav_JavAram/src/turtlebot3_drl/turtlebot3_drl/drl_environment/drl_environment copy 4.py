...
class DRLEnvironment(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        self.known_cells_set = set()        # set de (ix, iy) en coordenadas mundo discretas
    ...
    *******************************************************************************"""
    ...
    def _map_index_to_world_index(self, col, row, origin_x, origin_y, resolution):
        # coordenada del centro de la celda en mundo
        x = origin_x + (col + 0.5) * resolution
        y = origin_y + (row + 0.5) * resolution
        # discretizar en la misma rejilla (usar la misma resolución para keys)
        ix = int(round(x / resolution))
        iy = int(round(y / resolution))
        return (ix, iy)

    # Map callback: update map arrays and coverage (reemplaza tu map_callback)
    def map_callback(self, msg):
        ...
        # guardar resolución/origen actuales
        ...
        # calcular known cells actuales y su set en coordenadas mundo discretas
        ...
        # opcional: actualizar visit_counts si quieres (map local)
        # si el mapa cambió de tamaño o origen, visit_counts debe ser realineado; aquí asumimos que visit_counts
        # corresponde al grid actual y se reinicia si shape cambia
        ...
        # actualizar stable coverage counter (mantener tu lógica)
        ...
        # guardar para uso en step (si necesitas n_new local además del global)
    ...
    def get_state(self, action_linear_previous, action_angular_previous):
        ...
        # Collision
        #el
        if self.obstacle_distance < THRESHOLD_COLLISION: # obstacle_distance is the minmum distance from LiDAR, if it is below threshold, collision happened
