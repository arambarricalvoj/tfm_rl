...
# -------------------------
def get_reward_explore(succeed, action_linear, action_angular,
                       min_obstacle_dist, n_new_cells,
                       allow_discovery_reward=True, dist_since_last_reward=0.0,
                       extras=None):
    """
    Firma esperada para exploración:
      (succeed, action_linear, action_angular, min_obstacle_dist, n_new_cells,
       allow_discovery_reward=True, dist_since_last_reward=0.0, extras=None)
    Devuelve: float (recompensa)
    NOTA: llamar reward_initialize() en env.reset() para reiniciar E_filtered.
    """
    global E_filtered

    # 1) Suavizado exponencial del incremento de exploración
    lam = params.get('lambda_E', 0.3)
    E_filtered = (1.0 - lam) * E_filtered + lam * float(max(0.0, n_new_cells))

    # 2) Normalizaciones
    kappa = params.get('kappa_E', 50.0)
    E_norm = math.tanh(E_filtered / kappa) if allow_discovery_reward else 0.0
    v_norm = math.tanh(float(max(0.0, action_linear)) / params.get('v_max', 0.3))
    omega_norm = -math.tanh(abs(float(action_angular)) / params.get('omega_max', 1.5))
    s_norm = math.tanh(float(max(0.0, min_obstacle_dist)) / params.get('s_scale', 2.0))

    # 3) Penalización por proximidad suave
    prox_pen = 0.0
    if min_obstacle_dist < MIN_DIST_FOR_PROX_PEN:
        prox_pen = PROX_PENALTY * (1.0 - (min_obstacle_dist / MIN_DIST_FOR_PROX_PEN))

    # 4) Penalizaciones opcionales por revisitas o spinning (si extras las proporciona)
    revisit_pen = 0.0
    spin_pen = 0.0
    if extras:
        if 'revisit_ratio' in extras:
            revisit_pen = -REVISIT_DECAY * float(extras.get('revisit_ratio', 0.0))
        if 'spin_metric' in extras:
            spin_pen = -SPIN_PENALTY_SCALE * float(extras.get('spin_metric', 0.0))

    # 5) Combinación lineal
    r_raw = (params.get('alpha', 1.0) * E_norm +
             params.get('beta', 0.2)  * v_norm +
             params.get('gamma', -0.8) * omega_norm +
             params.get('delta', 0.6) * s_norm +
             prox_pen + revisit_pen + spin_pen)

    # 6) Terminales fuertes
    if succeed == SUCCESS:
        r_raw += 2.0
    elif succeed in (COLLISION_OBSTACLE, COLLISION_WALL):
        r_raw += COLLISION_PENALTY
    elif succeed == TIMEOUT:
        r_raw += TIMEOUT_PENALTY
    elif succeed == TUMBLE:
        r_raw += TUMBLE_PENALTY

    # 7) Clipping final
    r_clipped = max(params.get('r_min', -1.0), min(params.get('r_max', 1.0), r_raw))
